...
// Sensitive keys patterns (passwords, tokens, secrets)
const SENSITIVE_KEYS = /(password|pwd|token|secret|ssid)/i

function getKey() {
  const key = process.env.ENCRYPTION_KEY
  if (!key) throw new Error('ENCRYPTION_KEY environment variable is not set')
  // Accept hex or base64; normalize to 32-byte Buffer
  let buf
  if (/^[0-9a-fA-F]{64}$/.test(key)) {
    buf = Buffer.from(key, 'hex')
  } else {
    // assume base64 or utf8; try base64 first
    try {
  ...
  if (buf.length !== 32) {
    // last resort: truncate/pad to 32 bytes
